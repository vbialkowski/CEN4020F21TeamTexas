...
def InCollegeLearn():
    ...
    while choice != 6:
        if choice == 1:
            string = "How to use In College learning"
            learn(string)
            return
        elif choice == 2:
            string = "Train the trainer"
            learn(string)
            return
        elif choice == 3:
            string = "Gamification of learning"
            learn(string)
            return
        elif choice == 4:
            string = "Understanding the Architectural Design Process"
            learn(string)
            return
        elif choice == 5:
            string = "Process Management Simplified"
            learn(string)
            return
        else:
            print("Invalid Option!")
    ...
    for line in listOfLines:
        ...
        if iteration % 3 == 2:
            ...
            if not IsInDatabase and (accountCount() < 10):
                ...
                temp = accountCount()
                while temp != 0:
                    listOfLines[temp - 1] = listOfLines[temp - 1].rstrip() + "\t" + APIFirstName + " " + APILastName + "\n"
                    temp += -1
                memberfileread.close()
                memberfilewrite = open("NewMembers.txt", "w")
                memberfilewrite.writelines(listOfLines)
